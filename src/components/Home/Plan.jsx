    ...
    return (
        ...
        >
            <div className='max-w-[1240px] mx-auto p-4'>
                <h1 className='font-serif text-primary text-2xl md:text-5xl font-bold text-center'>Our Plans</h1>
                <p className='text-center pt-4 md:text-lg font-bold'>Choose Your Plan</p>
                <p className='text-center pt-2 text-primary md:text-lg text-bold'>We Provide The Best Services For The Most Reasonable Price</p>
            </div>
            ...
            <div className="max-w-[1240px] mx-auto p-4 md:grid grid-cols-3 gap-8">
                {inView && planData.map((item, index) => (
                    <div key={index} className="flex flex-col sm:mx-2 shadow-xl h-auto rounded-xl px-4 py-12 border border-primary my-5 hover:shadow-2xl hover:scale-105 duration-500">
                            ...
                            <ul>
                                {Object.keys(item).map((key, index) => {
                                    ...
                                    return item[key] && <li key={index} className='text-lg md:text-md py-3 flex items-center'>
                                        <span className='mr-2'><AiOutlineCheckCircle className='text-lg md:text-xl text-primary' /></span>
                                        {item[key]}
                                    </li>
                                })}
                            </ul>
                        ...
                        <div className='text-center mt-auto '>
                            <button className='border-2 border-primary mt-4 px-4 py-1 rounded-full text-primary hover:text-white hover:bg-primary duration-300'>Get Quote</button>
                        </div>
                    </div>
                ))}
            </div>
